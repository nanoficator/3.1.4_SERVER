...
function buildEditModal(user, allAuthorities) {
    ...
    for (i = 0; i < allAuthorities.length; i++) {
        content += '                            <option ' + (user.authorities.toString().includes(allAuthorities[i].toString()) ? 'selected' : '') + '>' + allAuthorities[i].name.substring(5) + '</option>';
        console.log(user.authorities.includes(allAuthorities[i]));
    }
